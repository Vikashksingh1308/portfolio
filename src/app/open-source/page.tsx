...
function PrState({ pr }: { pr: GitHubPR }) {
  ...
  if (pr.state === "open")
    return (
      <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full border border-[var(--accent)]/30 bg-[var(--accent)]/10 text-[var(--accent)]">
        <Circle size={10} /> Open
      </span>
    );
  return (
    <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full border border-[var(--border)] bg-[var(--surface-raised)] text-[var(--muted)]">
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-16">
      ...
      {prs.length === 0 ? (
        <div className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-10 text-center">
          <GitPullRequest size={24} className="mx-auto text-[var(--muted)] mb-3" />
          ...
          <Link
            ...
            className="inline-flex items-center gap-1.5 text-sm text-[var(--accent)] mt-3 hover:underline"
          >
